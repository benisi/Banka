...
describe('User should be able to login', function () {
  it('should give a status code of 200 and set user token variable', function (done) {
    _chai["default"].request(_app["default"]).post('/api/v1/auth/signin').send(_users.adminLoginData).end(function (err, res) {
...
describe('Test to create a user bank account', function () {
  it('should return a status code of 201, i.e account was created successfully', function (done) {
    _chai["default"].request(_app["default"]).post('/api/v1/accounts').set('Authorization', globalToken).send(_account.validAccountData).end(function (err, res) {
...
describe('Test to activate user account', function () {
  it('should return a status of 200', function (done) {
    _chai["default"].request(_app["default"]).patch("/api/v1/account/".concat(accNumb)).set('Authorization', globalToken).send(_account.activateAccountData).end(function (err, res) {
...
describe('Client should be able to login', function () {
  it('should give a status code of 200 and set user token variable', function (done) {
    _chai["default"].request(_app["default"]).post('/api/v1/auth/signin').send(_users.clientLoginData).end(function (err, res) {
      expect(res).to.have.status(200);
      var token = res.body.data.token;
      clientToken = token;
      done();
