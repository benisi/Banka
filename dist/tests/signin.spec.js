...
describe('Tests for valid input for user login', function () {
  it('should give a status code of 200', function (done) {
    _chai["default"].request(_server["default"]).post(url).send(_users.validLoginData).end(function (err, res) {
  ...
  it('should give a status code 401', function (done) {
    _chai["default"].request(_server["default"]).post(url).send(_users.invalidLoginData).end(function (err, res) {
...
describe('Tests for undefined email', function () {
  it('should give a status code of 400', function (done) {
    _chai["default"].request(_server["default"]).post(url).send(_users.undefinedEmailLoginData).end(function (err, res) {
      expect(res).to.have.status(400);
      done();
    });
  });
});
describe('Tests for undefined password', function () {
  it('should give a status code of 400', function (done) {
    _chai["default"].request(_server["default"]).post(url).send(_users.undefinedPasswordLoginData).end(function (err, res) {
      expect(res).to.have.status(400);
      done();
    });
  });
